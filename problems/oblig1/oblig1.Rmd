...
\text{$Y_i$} is the response for the i'th data point, and each data point is a row of zeroes, except where \text{$c_i = j$}. Looking at the two models we first set \text{$x_{i1} = 1$}, and thus all other \text{$x_{ij} = 0$ for $ j = 2, 3, ... K$}. If the two models are equal, this implies that \text{$\beta_0 = \alpha_1$}. Setting \text{$x_{i2} = 1$} giving all other \text{$x_{ij} = 0$ for $ j = 1, 3, 4, ... K$} and thus \text{$\beta_0 + \beta_2 = \alpha_2$}. We can do this in general setting \text{$x_{il} = 1$}  thus \text{$x_{ij} = 0$ for $ j = 1, 2, ... , l-1, l+1, ... ,K$} giving the relationship \text{$\beta_j + \beta_0 = \alpha_j$}. The relationship between the \text{$\alpha$} and \text{$\beta$} coefficients is thus.
...
$$
 ...
 $$
 \alpha_l = \gamma_0 + \gamma_l \\ \gamma_l = \alpha_l - \gamma_0 = \alpha_l - \bar{\alpha} 
 $$
...
summary(fit1)
```
 This result is more reasonable as each of the 4 categories has its own coefficient. The actual numerical values 1,2,3,4 does not effect the model which was the case before we made them categorical. In the \textt{lm} command, we enforced the fit to not include a intercept (+0). This implies that this fit corresponds to a model that does not have any baseline (or a baseline of zero). It is thus the \text{$\alpha$} model.
